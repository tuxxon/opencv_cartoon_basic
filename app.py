...
def lambda_handler(event, context):
    ...
    conv_filename='/tmp/cartoon_basic{}'.format(ext)
    down_jsonfile='/tmp/cartoon_basic.json'

    #
    # S3 files
    #
    s3_filename='public/{hash}/cartoon-basic{ext}'.format(hash=h,ext=ext)
    s3_paramfile='public/{hash}/cartoon-basic.json'.format(hash=h) 
    ...
    output = render_cartoon.render_basic(down_filename, blockSize, C)
